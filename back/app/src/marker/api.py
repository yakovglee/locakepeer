...
async def create_markers(item: schemas.MarkerCreate, db: Session = Depends(get_db)):
    created_item = await crud.create_marker(db=db, item=item)
    return created_item


@router.get("/{marker_id}", response_model=schemas.Marker)
async def read_marker(marker_id: int, db: Session = Depends(get_db)):
...
async def upd_markerid_in_place(
    place_id: int, marker_id: int, db: Session = Depends(get_db)
):
    db_item = await mPlace_upd_markerid(db=db, place_id=place_id, marker_id=marker_id)
    if db_item is None:
...
async def read_markers(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
